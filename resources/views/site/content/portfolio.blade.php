        ...
        <div id="filters" class="sixteen columns">
            <ul class="clearfix">
                ...
                @foreach($filters as $filter)
                    <li><a class="" href="#" data-filter=".{{ $filter->title }}">
                            <h5>{{ $filter->title }}</h5>
                        </a></li>
                @endforeach
            </ul>
        ...
        @foreach($portfolio as $item)
            <!-- Portfolio Item -->
                <div style="position: absolute; left: 0px; top: 0px; transform: translate3d(0px, 0px, 0px) scale3d(1, 1, 1); width: 337px; opacity: 1;"
                     class="portfolio-item one-four {{ $item->filter->title }} isotope-item">
                    <div class="portfolio_img">{!! Html::image('assets/img/' . $item->image, $item->name) !!}
                    </div>
